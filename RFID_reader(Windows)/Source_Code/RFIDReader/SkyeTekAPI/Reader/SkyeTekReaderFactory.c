  ...
  )
{
   ...
   int i = 0;
   unsigned int ix = 0;
   LPSKYETEK_PROTOCOL lpProt = NULL; // JDK0
   ...
   if( ver == 3 )
   {
      ...
      if( status == SKYETEK_SUCCESS && lpData->data != NULL && lpData->size >= 2 )
      {
         ...
         case 0x0A:
            ...
            break;
         case 0x10:
            _tcscpy(lpReader->model, _T("Gemini"));
            break;
         default:
            _tcscpy(lpReader->model, _T("??"));
            break;
   ...
   lpProt = lpReader->lpProtocol; // JDK0
   ...
   SkyeTek_FreeID(tmpReader.id); // JDK0

   return lpReader;

failure:
   SkyeTek_FreeData(lpData); // JDK0
   if ( lpProt ) // JDK0
      free(lpProt);
  ...
  )
{
   ...
   for(ix = 0; ix < deviceCount; ix++)
   {
      ...
      if( lpDI->Open(devices[ix]) == SKYETEK_SUCCESS )
      {
         ...
         {
            int retval = 0;
            if((retval = SkyetekReaderFactory_CreateReader(devices[ix], &lpReader)) == SKYETEK_SUCCESS)
            {
               *readers = (LPSKYETEK_READER*)realloc(*readers, (readerCount + 1)*sizeof(LPSKYETEK_READER));
               (*readers)[readerCount] = lpReader;
               readerCount++;
            ...
            {
               SkyeTek_Debug(_T("Creating reader failed %d\n"), retval);
            }
         }
         if( !found )
         {
  ...
  )
{
   ...
   {
      SkyeTek_FreeData((LPSKYETEK_DATA)(lpReader->id)); // JDK0
      free(lpReader->lpProtocol); // JDK0
      free(lpReader);
      return 1;
   }
   return 0;
